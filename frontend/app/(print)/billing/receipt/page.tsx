...
import { readBillingReceiptPrintDraft } from "@/utils/billing-receipt-print";

function PrintPageSkeleton() {
  return (
    <div className="animate-pulse rounded-[28px] border border-[#c8e4de] bg-white p-6 shadow-sm">
      <div className="mx-auto max-w-[7.4in] space-y-5">
        ...
        <div className="h-56 rounded-2xl border border-slate-200 bg-slate-50" />
...
export default function BillingReceiptPrintPage() {
  ...
  const draft = useSyncExternalStore(
    () => () => {},
    () => (draftId ? readBillingReceiptPrintDraft(draftId) : null),
    () => undefined
  );
        ...
        <div className="mx-auto max-w-5xl space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-[#133d37]">Print Payment Receipt</h1>
            </div>
          ...
          {isLoading ? (
            ...
            <BillingReceiptDocument receipt={draft} />
          ) : (
            <div className="rounded-3xl border border-[#f0c6c0] bg-white px-6 py-10 text-center text-sm text-[#9a4f45]">
              Unable to load the billing receipt draft. Reopen the billing record and try again.
            </div>
          )}
        </div>
      </div>
