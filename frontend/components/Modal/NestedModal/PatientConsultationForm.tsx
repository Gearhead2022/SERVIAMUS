...
function VitalsRow({ label, teal = false, form, prefix } : {label: string, teal?: boolean,  form: StepProps["form"], prefix: "prev" | "curr";}) {
  const { register } = form;
  const fields = [
    { name: "bp", label: "BP (mmHg)", ph: "120/80" },
    { name: "temp", label: "Temp (°C)", ph: "36.6" },
    { name: "pulse", label: "Pulse (bpm)", ph: "72" },
    { name: "rr", label: "RR (/min)", ph: "16" },
    { name: "weight", label: "Weight (kg)", ph: "60" },
    { name: "height", label: "Height (cm)", ph: "165" },
  ];
  return (
    <div className="mb-5">
      <h4
      ...
      <div className="grid grid-cols-6 gap-2">
        {fields.map((f) => (
          <div key={f.name}>
            ...
            <input
              type="text"
              {...register(`${prefix}_${f.name}` as Path<RegisterConsultationFormValues>)}
              placeholder={f.ph}
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-x-7 gap-y-4">
        ...
        <FormGroup label="Date of Consultation">
          <input type="date" className={`${inputCls} ${errors.consultation_date ? '' : ''}`} {...register("consultation_date")}/>
...
function Step2({form} : StepProps) {
  ...
  return (
    <>
      ...
      <VitalsRow label="Previous Record" form={form} prefix="prev" />
      <VitalsRow label="Current Record" form={form} prefix="curr" teal />
      ...
      <div className="mt-2">
        ...
        />
         {errors.history_present_illness && (
            <p>{errors.history_present_illness.message as string}</p>
          )}
      </div>
    </>
  );
...
function Step3({form} : StepProps) {
  ...
  const personal_history = [
    { name: "travelHistory", label: "Travel History", ph: "Recent travel destinations" },
...
const PatientConsultationForm: React.FC<{patient: PatientProps | null, onClose: () => void}> = ({patient, onClose}) => {
    const [step, setStep] = useState(1);
    const { mutateAsync: registerPatient, isPending } = useConsultaion();
    ...
    function mapToPrisma(data: RegisterConsultationFormValues) {
      return {
        ...
        prev_bp: data.prev_bp,
        prev_temp: data.prev_temp,
        prev_pulse: data.prev_pulse,
        prev_rr: data.prev_rr,
        prev_weight: data.prev_weight,
        prev_height: data.prev_height,

        curr_bp: data.curr_bp,
        curr_temp: data.curr_temp,
        curr_pulse: data.curr_pulse,
        curr_rr: data.curr_rr,
        curr_weight: data.curr_weight,
        ...
        travel_history: data.travelHistory, // ⚠️ mapped
        diet: data.diet,
        stress: data.stress,
  ...
  return (
    <>
     <form
        ...
        noValidate
      >
      <div className="bg-white rounded-2xl w-full shadow-2xl overflow-y-scroll h-[80vh]">
        {/* Step Indicator */}
        <div className="bg-[#f7f8fc] border-b border-[#dce3ef] px-10 flex">
          {STEPS.map((s, i) => {
            ...
            return (
              <div
                key={n}
                className={`flex-1 flex items-center gap-2.5 py-4 relative ${
                  i < STEPS.length - 1
              ...
              >
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-semibold border-2 flex-shrink-0 transition-all duration-300 ${
                    isActive
                ...
                <div className="leading-tight">
                  <div
                    className={`text-[11.5px] font-${isActive ? "600" : "500"} ${
                      isActive ? "text-[#1a2a45]" : isDone ? "text-[#0e7c7b]" : "text-[#6b7da0]"
                    }`}
                  >
                    {s.label}
                  </div>
