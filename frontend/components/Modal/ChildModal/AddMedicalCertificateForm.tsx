...
import { MedCertFormValues, medCertSchema } from "@/schemas/consultation.schema";
import { normalizeMedCertDefaults } from "@/utils/consultation/normalizeMedCertDefaults";
...
interface MedCertProps {
    ...
    onClose: () => void;
    onPreview: (data: MedCertFormValues) => void;
}
...
const MedicalCertificate: React.FC<MedCertProps> = ({
    ...
    const form = useForm<MedCertFormValues>({
        resolver: zodResolver(medCertSchema),
        defaultValues: normalizeMedCertDefaults(mcrId, patient, requestEntry),
    });
        ...
        <div className="flex flex-col font-['DM_Sans'] bg-white">
            ...
            <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 flex-wrap gap-3"
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <Button variant="primary" type="button" onClick={handlePreview} >Preview</Button>
                </div>
            </div>
        </div >
    );
